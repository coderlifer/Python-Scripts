...
class KMeans(object):
    def __init__(self, k=4, distance='l2', max_iter=800, centroids=None, data=None):
        self.k = k
    ...
    def l2_dist(self, data):
        ...
        square_rs = np.subtract(np.add(data_square_expand, centroids_square_expand), multiply_rs * 2)
    ...
    def update_centroids(self, dists):
        ...
        print(dists.shape)
        self.labels = np.reshape(np.argsort(dists, axis=1)[:, 0], (-1, 1)) # m * n
